...
if __name__ == "__main__":
    if not OAUTH_TOKEN:
        ...
        outfile = open('paradox_twitter_FC_b.txt', 'w')
        r = requests.get(url='https://api.twitter.com/1.1/friends/list.json?cursor=-1&count=200&screen_name=' + 'cosmic_meow', auth = oauth)

        followers_count = 0

        for persons in r.json()['users']:
        	username = str(persons['screen_name'])
        	followers = str(persons['friends_count'])
